...
import { IsNotEmpty, IsString, IsUUID, IsOptional, IsInt, IsPositive, Min } from "class-validator";
...
@InputType()
export class UpdateMaterialRequestItemInput extends PartialType(CreateMaterialRequestItemInput) {
    @Field(() => ID)
    @IsNotEmpty({ message: 'El ID del item de solicitud de material es requerido' })
    @IsUUID()
    id: string;
}
