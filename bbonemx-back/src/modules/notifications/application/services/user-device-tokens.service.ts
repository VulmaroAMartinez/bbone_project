...
export class UserDeviceTokensService {
  ...
  async getTokensForUsers(
    userIds: string[],
  ): Promise<Map<string, UserDeviceToken[]>> {
  ...
  async register(
    ...
    const existing = await this.tokensRepository.findByUserAndToken(
      userId,
      input.fcmToken,
    );
    if (existing) {
    ...
    const otherUserToken = await this.tokensRepository.findByToken(
      input.fcmToken,
    );
    if (otherUserToken) {
      this.logger.debug(
        `Token reasignado de usuario ${otherUserToken.userId} a ${userId}`,
      );
