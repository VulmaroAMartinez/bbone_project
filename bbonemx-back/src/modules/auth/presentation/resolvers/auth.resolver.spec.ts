...
describe('AuthResolver', () => {
  ...
  const resolver = new AuthResolver(authService as any, configService);
  ...
  it('login autentica usuario y retorna payload de login', async () => {
    ...
    const result = await resolver.login(
      ...
      { req: { ip: '127.0.0.1', headers: { 'user-agent': 'jest' } }, res: resCookie } as any,
    );

    expect(authService.authenticateUser).toHaveBeenCalledWith('1001', 'secret123', '127.0.0.1');
    expect(result).toEqual({ user, accessToken: 'a' });
  });

  it('refreshAuth regresa false sin refresh token', async () => {
    const result = await resolver.refreshAuth({ req: { cookies: {} } } as any);
  ...
  it('logout revoca refresh token y responde true', async () => {
    ...
    const result = await resolver.logout({ req: { cookies: { refresh_token: 'r1' } }, res: { clearCookie: jest.fn() } } as any);
