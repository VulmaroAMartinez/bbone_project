...
export class WorkOrderTechniciansService {
  ...
  async assignMetadata(
    ...
    const existing =
      await this.woTechniciansRepository.findByWorkOrderAndTechnician(
        input.workOrderId,
        input.technicianId,
      );

    if (existing)
      throw new ConflictException('El técnico ya está asignado a esta OT');

    if (input.isLead)
      await this.woTechniciansRepository.removeLeadFromAll(input.workOrderId);
  ...
  async updateAssignment(
    ...
    if (input.isLead)
      await this.woTechniciansRepository.removeLeadFromAll(
        woTechnician.workOrderId,
      );
  ...
  async unassign(workOrderId: string, technicianId: string): Promise<void> {
    ...
    const count =
      await this.woTechniciansRepository.countByWorkOrderId(workOrderId);

    if (count <= 1)
      throw new BadRequestException(
        'No se puede desasignar el único técnico asignado a la OT',
      );

    await this.woTechniciansRepository.softDelete(assignment.id);
  }

  async isTechnicianAssigned(
    workOrderId: string,
    technicianId: string,
  ): Promise<boolean> {
    return this.woTechniciansRepository.isTechnicianAssigned(
      workOrderId,
      technicianId,
    );
  }
