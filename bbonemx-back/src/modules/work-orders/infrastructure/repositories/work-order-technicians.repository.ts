...
export class WorkOrderTechniciansRepository {
  ...
  async findByWorkOrderAndTechnician(
    workOrderId: string,
    technicianId: string,
  ): Promise<WorkOrderTechnician | null> {
  ...
  async findById(id: string): Promise<WorkOrderTechnician | null> {
    return this.repository.findOne({
      ...
      relations: [
        ...
        'assigner',
        'assigner.userRoles',
        'assigner.userRoles.role',
      ],
    });
  }

  async findLeadByWorkOrderId(
    workOrderId: string,
  ): Promise<WorkOrderTechnician | null> {
    return this.repository.findOne({
  ...
  async create(
    data: Partial<WorkOrderTechnician>,
  ): Promise<WorkOrderTechnician> {
  ...
  async update(
    ...
    const workOrderTechnician = await this.repository.findOne({
      where: { id },
    });
  ...
  async removeLeadFromAll(workOrderId: string): Promise<void> {
    await this.repository.update(
      { workOrderId: workOrderId, isLead: true, isActive: true },
      { isLead: false },
    );
  }
