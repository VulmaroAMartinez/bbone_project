...
export class WorkOrdersResolver {
  ...
  async findFiltered(
    ...
    if (
      user?.hasRole(Role.REQUESTER) &&
      !user?.hasRole(Role.ADMIN) &&
      !user?.hasRole(Role.BOSS)
    ) {
      scopedFilters.requesterId = user.id;
    }

    if (
      user?.hasRole(Role.TECHNICIAN) &&
      !user?.hasRole(Role.ADMIN) &&
      !user?.hasRole(Role.BOSS)
    ) {
    ...
    const { data, total } = await this.workOrdersService.findWithFilters(
      scopedFilters,
      pagination || { page: 1, limit: 20 },
      sort || { field: WorkOrderSortField.CREATED_AT, order: SortOrder.DESC },
    );
  ...
  @UseGuards(RolesGuard)
  @Roles(Role.ADMIN)
  resumeWorkOrder(@Args('id', { type: () => ID }) id: string) {
  ...
  @UseGuards(RolesGuard)
  @Roles(Role.ADMIN, Role.TECHNICIAN, Role.REQUESTER)
  signWorkOrder(
  ...
  @UseGuards(RolesGuard)
  @Roles(Role.TECHNICIAN)
  async removeWorkOrderSparePart(
    @Args('id', { type: () => ID }) id: string,
  ): Promise<boolean> {
  ...
  getPhotosBeforeStart(@Parent() workOrder: WorkOrder) {
    return this.workOrderPhotosService.findByWorkOrderAndPhotoType(
      workOrder.id,
      PhotoType.BEFORE,
    );
  }
  ...
  private async assertCanAccessWorkOrder(
    user: User,
    workOrder: WorkOrder,
  ): Promise<void> {
    ...
    if (user.hasRole(Role.TECHNICIAN)) {
      const isAssigned =
        await this.workOrderTechniciansService.isTechnicianAssigned(
          workOrder.id,
          user.id,
        );
      if (isAssigned) {
