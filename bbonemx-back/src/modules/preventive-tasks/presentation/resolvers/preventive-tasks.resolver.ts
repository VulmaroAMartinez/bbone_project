...
export class PreventiveTasksResolver {
  ...
  @Query(() => PreventiveTaskPaginatedResponse, {
    name: 'preventiveTasksFiltered',
  })
  async findFiltered(
    ...
    const { data, total } = await this.preventiveTasksService.findWithFilters(
      filters,
      pagination,
      sort,
    );
  ...
  @Query(() => PreventiveTaskType, {
    name: 'preventiveTaskById',
    nullable: true,
  })
  ...
  update(
    ...
    @Args('input') input: UpdatePreventiveTaskInput,
  ) {
  ...
  close(
    @Args('id', { type: () => ID }) id: string,
    @Args('input') input: ClosePreventiveTaskInput,
  ) {
    return this.preventiveTasksService.close(id, input);
  }

  @Mutation(() => GenerateWorkOrdersResult, {
    name: 'generateWorkOrdersForPreventiveTask',
  })
  generateWorkOrders() {
