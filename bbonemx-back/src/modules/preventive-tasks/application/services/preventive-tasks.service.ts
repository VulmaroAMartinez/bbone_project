...
export class PreventiveTasksService {
  ...
  findWithFilters(
    ...
    return this.preventiveTasksRepository.findWithFilters(
      filters,
      pagination,
      sort,
    );
  ...
  async update(
    ...
    if (task.status === PreventiveTaskStatus.CLOSED)
      throw new BadRequestException('La tarea preventiva ya está cerrada');
  ...
  async activate(id: string): Promise<PreventiveTask> {
    const task =
      await this.preventiveTasksRepository.findByIdIncludeInactive(id);
  ...
  async generateDueWorkOrders(): Promise<{
    generated: number;
    tasks: string[];
  }> {
  ...
  async generateWorkOrderForTask(
    task: PreventiveTask,
  ): Promise<PreventiveTask> {
    ...
    if (!task.machine) {
      ...
      if (!fullTask?.machine)
        throw new BadRequestException(
          'La tarea preventiva no tiene máquina asociada',
        );
      task = fullTask;
    }
