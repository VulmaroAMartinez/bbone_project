...
export class AbsenceReasonsService {
  constructor(
    private readonly absenceReasonsRepository: AbsenceReasonsRepository,
  ) {}
  ...
  async findByIdOrFail(id: string): Promise<AbsenceReason> {
    ...
    if (!absenceReason) {
      throw new NotFoundException(
        `Razón de ausencia con ID ${id} no encontrada`,
      );
    }
  ...
  async deactivate(id: string) {
    await this.findByIdOrFail(id);
    await this.absenceReasonsRepository.softDelete(id);
    return this.findByIdOrFail(id);
  }
  async activate(id: string) {
    await this.findByIdOrFail(id);
    await this.absenceReasonsRepository.restore(id);
    return this.findByIdOrFail(id);
  }
}
